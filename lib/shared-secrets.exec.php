<?php
  ...
  {
      return trail(SECRET_SHARING_URL, "/").$secret;
  }
  ...
  {
      return base64_decode(url_base64_decode(nolead($url, trail(SECRET_SHARING_URL, "/"))), true);
  }
  ...
  function url_base64_decode($url_base64_content)
  {
      ...
      if (is_string($url_base64_content)) {
          $result = str_replace(
              [URL_BASE64_MARKER_A, URL_BASE64_MARKER_B],
              [BASE64_MARKER_A, BASE64_MARKER_B],
              $url_base64_content
          );
                          ...
                          if ((false !== $data[OPENSSL_ENCKEY]) && (false !== $data[OPENSSL_MACKEY])) {
                              ...
                              if (false !== $data[OPENSSL_CHECKMAC]) {
                                  if (hash_equals($data[OPENSSL_CHECKMAC], $data[OPENSSL_MAC])) {
                                      ...
                                      $data[OPENSSL_MESSAGE] = openssl_decrypt(
                                          $data[OPENSSL_ENCMESSAGE],
                                          "aes-256-ctr",
                                          $data[OPENSSL_ENCKEY],
          ...
          try {
              ...
              if (118 <= strlen($data[OPENSSL_FULLMESSAGE])) {
                  ...
                  for ($i = 0; $i < $data[OPENSSL_RSAKEYCOUNT]; $i++) {
                      ...
                      $data[OPENSSL_RSAKEYS][$i]       = substr(
                          $data[OPENSSL_MACMESSAGE],
                          $position+34,
                  ...
                  if (false !== $data[OPENSSL_KEY]) {
                      ...
                      if ((false !== $data[OPENSSL_ENCKEY]) && (false !== $data[OPENSSL_MACKEY])) {
                          ...
                          if (false !== $data[OPENSSL_NONCE]) {
                              ...
                              $data[OPENSSL_ENCMESSAGE] = openssl_encrypt(
                                  $data[OPENSSL_MESSAGE],
                                  "aes-256-ctr",
                                  $data[OPENSSL_ENCKEY],
                                  OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING,
                                  $data[OPENSSL_NONCE]
