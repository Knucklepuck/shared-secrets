<?php
  ...
  define("MYSQL_READ_SHARE", "SELECT encrypted_secret FROM secrets WHERE uri = ?");
  define("MYSQL_WRITE", "INSERT IGNORE INTO secrets (keyid, fingerprint, time) VALUES (?, ?, CURRENT_TIMESTAMP)");
  define("MYSQL_WRITE_SHARE", "INSERT IGNORE INTO secrets (encrypted_secret, uri, time) VALUES (?, ?, CURRENT_TIMESTAMP)");
  define("MYSQL_WRITE_USE", "UPDATE IGNORE secrets SET keyid = ?, fingerprint = ?, encrypted_secret = '', uri = '', time = CURRENT_TIMESTAMP WHERE encrypted_secret = ?");
